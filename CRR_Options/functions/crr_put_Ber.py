def put_price_Ber(S0, K, T, u, d, r):
    """
    put_price(S0, K, T, u, d, r) = initial price of a Bermudan put option in CRR method
    ...
    for i in range(T - 1, -1, -1):
        for j in range(0, len(price) - 1):
            price[j] = max(((q * price[j]) + (1 - q) * price[j + 1]) / (1 + r),
                           max(0, K - S0 * (1 + u)**(i - j) * (d + 1)**j))
        price.pop()
